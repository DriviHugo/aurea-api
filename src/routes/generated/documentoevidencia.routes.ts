...
const routes: FastifyPluginAsync = async (fastify) => {
  ...
  fastify.get('/:id', {
    ...
    try {
      ...
      if (!documentoEvidencia) {
        return reply.status(404).send({ error: 'Documento-evidencia relationship not found' });
      }
    ...
    try {
      ...
      if (updateData.documentoId || updateData.evidenciaId) {
        ...
        if (updateData.documentoId) {
          checks.push(
            prisma.documento.findUnique({ where: { id: updateData.documentoId } })
        ...
        if (updateData.evidenciaId) {
          checks.push(
            prisma.evidencia.findUnique({ where: { id: updateData.evidenciaId } })
  ...
  fastify.delete('/:id', {
    ...
    try {
      ...
      return reply.status(200).send({ message: 'Documento-evidencia relationship deleted successfully' });
    } catch (error) {
      fastify.log.error(error);
      return reply.status(500).send({ error: 'Internal server error' });
    }
...
1. **Complete CRUD operations** for DocumentoEvidencia model
2. **Pagination support** with page/limit parameters and filtering by documentoId/evidenciaId
3. **Proper error handling** including 404, 400, and 500 status codes
4. **Authentication** using `fastify.authAccessToken` preValidation hook
5. **AJV schema validation** for all request bodies, params, and query strings
6. **Swagger/OpenAPI documentation** with comprehensive schemas and responses
7. **Relationship validation** to ensure referenced Documento and Evidencia exist
8. **Unique constraint handling** for the composite unique key
9. **Proper TypeScript typing** throughout
10. **Include statements** to fetch related documento and evidencia data
11. **Production-ready error handling** with proper logging

The routes handle the junction table nature of DocumentoEvidencia appropriately, validating that both referenced entities exist before creating relationships and providing meaningful error messages for constraint violations.