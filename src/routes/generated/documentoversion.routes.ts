...
const routes: FastifyPluginAsync = async (fastify) => {
  ...
  fastify.get('/:id', {
    ...
    try {
      ...
      if (!documentoVersion) {
        return reply.status(404).send({ error: 'Documento version not found' });
      }
  ...
  // POST /documento-versions - Create new documento version
  fastify.post('/', {
    preValidation: [fastify.authAccessToken],
    schema: {
      tags: ['DocumentoVersion'],
  ...
  // PUT /documento-versions/:id - Update documento version
  fastify.put('/:id', {
    preValidation: [fastify.authAccessToken],
    schema: {
      tags: ['DocumentoVersion'],
      description: 'Update documento version',
      params: idParamsSchema,
    ...
    try {
      ...
      const existingDocumentoVersion = await prisma.documentoVersion.findUnique({
        where: { id }
      });
  ...
  // DELETE /documento-versions/:id - Delete documento version
  fastify.delete('/:id', {
    preValidation: [fastify.authAccessToken],
    schema: {
      tags: ['DocumentoVersion'],
      description: 'Delete documento version',
      params: idParamsSchema,
      response: {
        200: {
          type: 'object',
  ...
  fastify.get('/documento/:documentoId', {
    preValidation: [fastify.authAccessToken],
    schema: {
      tags: ['DocumentoVersion'],
    ...
    try {
      ...
      const [documentoVersions, total] = await Promise.all([
        ...
        prisma.documentoVersion.count({ where: { documentoId } })
      ]);
