...
const routes: FastifyPluginAsync = async (fastify) => {
  ...
  fastify.post('/', {
    ...
    try {
      ...
      if (error.code === 'P2002') {
        return reply.status(400).send({ error: 'Version already exists for this documento' });
      }
      if (error.code === 'P2003') {
        return reply.status(400).send({ error: 'Referenced documento or usuario does not exist' });
      }
      return reply.status(500).send({ error: 'Internal server error' });
    }
  });
  ...
  fastify.delete('/:id', {
    ...
    try {
      ...
      return reply.status(200).send({ message: 'Documento version deleted successfully' });
    } catch (error) {
      return reply.status(500).send({ error: 'Internal server error' });
    }
  });
