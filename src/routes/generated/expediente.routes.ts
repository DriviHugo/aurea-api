...
const expedienteSchema = {
  ...
  properties: {
    ...
    procedimientoPropuesto: { 
      type: 'string',
      enum: ['abierto', 'restringido', 'competitivo_con_negociacion', 'procedimiento_negociado', 'dialogo_competitivo', 'asociacion_innovacion']
    },
...
const expedienteResponseSchema = {
  ...
  properties: {
    ...
    importeProrrogas: { type: 'number' },
    importeModificados: { type: 'number' },
    procedimientoPropuesto: { type: 'string' },
    procedimientoSeleccionado: { type: 'string' },
    cpvElegido: { type: 'string' },
    tieneLotes: { type: 'boolean' },
    justificacionLotes: { type: 'string' },
    numLotes: { type: 'integer' },
...
const routes: FastifyPluginAsync = async (fastify) => {
  ...
  // GET /expedientes - List expedientes with pagination
  fastify.get('/', {
    preValidation: [fastify.authAccessToken],
    schema: {
      tags: ['Expedientes'],
      description: 'Get list of expedientes with pagination',
  ...
  fastify.put('/:id', {
    ...
    try {
      ...
      const updatedExpediente = await prisma.expediente.update({
        ...
        data: {
          ...data,
          metadatos: data.metadatos || existingExpediente.metadatos
        }
      });
