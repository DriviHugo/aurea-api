...
const routes: FastifyPluginAsync = async (fastify) => {
  ...
  fastify.post('/', {
    ...
    try {
      const { email, name, password, imageUrl, isAdmin = false, isActive = false } = request.body as {
        email: string;
        name: string;
        password: string;
        imageUrl?: string;
    ...
    try {
      ...
      if (existingUser.SessionEntity.length > 0) {
        return reply.status(400).send({ 
          error: 'Cannot delete user with active sessions. Please delete sessions first.' 
        });
      }
