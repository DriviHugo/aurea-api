...
const routes: FastifyPluginAsync = async (fastify) => {
  ...
  fastify.delete('/:id', {
    ...
    try {
      ...
      const totalRelatedRecords = expedientesCount + documentosCount + validacionesCount + 
                                 revisionesCount + comentariosCount + auditLogsCount + 
                                 reglasCount + funcionVersionesCount;

      if (totalRelatedRecords > 0) {
        return reply.status(400).send({ 
          error: 'Cannot delete profile with associated records. Consider deactivating instead.' 
        });
      }
      ...
      return reply.status(200).send({ message: 'Profile deleted successfully' });
    } catch (error) {
      fastify.log.error(error);
      return reply.status(500).send({ error: 'Internal server error' });
    }
