...
const routes: FastifyPluginAsync = async (fastify) => {
  ...
  fastify.post('/', {
    ...
    try {
      ...
      if (error.code === 'P2002') {
        return reply.status(400).send({ error: 'Profile with this ID already exists' });
      }
      return reply.status(500).send({ error: 'Internal server error' });
    }
  });
  ...
  fastify.delete('/:id', {
    ...
    try {
      ...
      if (error.code === 'P2003') {
        return reply.status(400).send({ error: 'Cannot delete profile with existing references' });
      }
      return reply.status(500).send({ error: 'Internal server error' });
    }
  });
