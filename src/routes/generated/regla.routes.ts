...
const routes: FastifyPluginAsync = async (fastify) => {
  ...
  const reglaSchema = {
    ...
    properties: {
      ...
      aprobadorRol: { type: ['string', 'null'], enum: ['ADMIN', 'SUPERVISOR', 'USUARIO'] },
  ...
  const createReglaSchema = {
    type: 'object',
    required: ['codigo', 'nombre', 'descripcion', 'severidad', 'condicion', 'mensaje'],
    properties: {
  ...
  fastify.post('/', {
    ...
    try {
      ...
      if (existingRegla) {
        return reply.status(400).send({ error: 'Regla with this codigo already exists' });
      }
  ...
  fastify.delete('/:id', {
    ...
    try {
      ...
      return reply.status(200).send({ message: 'Regla deleted successfully' });
    } catch (error) {
      fastify.log.error(error);
      return reply.status(500).send({ error: 'Internal server error' });
    }
