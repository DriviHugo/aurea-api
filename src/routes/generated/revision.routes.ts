...
const routes: FastifyPluginAsync = async (fastify) => {
  ...
  fastify.get('/', {
    ...
    try {
      const { page = 1, limit = 10, expedienteId, usuarioId } = request.query as any;
    ...
    schema: {
      ...
      body: {
        ...
        properties: {
          ...
          usuarioId: { type: 'string', format: 'uuid' },
          rol: { type: 'string', enum: ['ADMINISTRADOR', 'COORDINADOR', 'ANALISTA', 'CONSULTOR'] },
          cambioDescripcion: { type: 'string', minLength: 1, maxLength: 1000 },
          decision: { type: 'string', enum: ['APROBADO', 'RECHAZADO', 'PENDIENTE', 'REVISION'] },
