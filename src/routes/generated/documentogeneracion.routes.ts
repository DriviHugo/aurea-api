  ...
  fastify.put(
    ...
    async (request, reply) => {
      try {
        ...
        const updatedDocumentoGeneracion =
          await prisma.documentoGeneracion.update({
            where: { id },
            data: {
              documentoId:
                data.documentoId !== undefined
                  ? data.documentoId
                  : existingDocumentoGeneracion.documentoId,
              expedienteId:
                data.expedienteId !== undefined
                  ? data.expedienteId
                  : existingDocumentoGeneracion.expedienteId,
              version:
                data.version !== undefined
                  ? data.version
                  : existingDocumentoGeneracion.version,
              plan:
                data.plan !== undefined
                  ? data.plan
                  : existingDocumentoGeneracion.plan,
            },
          });
    ...
    async (request, reply) => {
      try {
        ...
        if (error.code === "P2003") {
          return reply
            .status(400)
            .send({ error: "Cannot delete: foreign key constraint" });
        }
        return reply.status(500).send({ error: "Internal server error" });
      }
    },
