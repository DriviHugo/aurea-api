...
const routes: FastifyPluginAsync = async (fastify) => {
  ...
  fastify.post('/', {
    ...
    try {
      const { functionCode, functionName, providerName, modelo, inputVariables = {} } = request.body as {
        functionCode: string;
      ...
      return reply.status(400).send({ error: 'Failed to create AI function log' });
    }
  });
  ...
  fastify.put('/:id', {
    ...
    try {
      ...
      return reply.status(400).send({ error: 'Failed to update AI function log' });
    }
  });
