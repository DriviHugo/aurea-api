    ...
    async (request, reply) => {
      try {
        ...
        if (!documentoSeccion) {
          return reply
            .status(404)
            .send({ error: "Documento seccion not found" });
        }
      ...
      try {
        ...
        if (existingSeccion) {
          return reply.status(400).send({
            error: "A section with this order already exists for this document",
          });
        }
      ...
      try {
        ...
        if (data.orden && data.orden !== existingSeccion.orden) {
          ...
          if (conflictingSeccion) {
            return reply.status(400).send({
              error:
                "A section with this order already exists for this document",
            });
          }
        }
        ...
        const documentoSeccion = await prisma.documentoSeccion.update({
          ...
          data: {
            ...
            ...(data.tokensUsados !== undefined && {
              tokensUsados: data.tokensUsados,
            }),
            ...(data.tiempoGeneracionMs !== undefined && {
              tiempoGeneracionMs: data.tiempoGeneracionMs,
            }),
            ...(data.articulosLcsp !== undefined && {
              articulosLcsp: data.articulosLcsp,
            }),
          },
          include: {
