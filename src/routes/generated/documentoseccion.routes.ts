...
const routes: FastifyPluginAsync = async (fastify) => {
  ...
  fastify.get('/:id', {
    ...
    try {
      ...
      if (!documentoSeccion) {
        return reply.status(404).send({ error: 'DocumentoSeccion not found' });
      }
    ...
    schema: {
      ...
      body: {
        ...
        properties: {
          ...
          tokensUsados: { type: 'integer' },
          tiempoGeneracionMs: { type: 'integer' },
    ...
    try {
      ...
      const documentoSeccion = await prisma.documentoSeccion.create({
        data: {
          ...
          tokensUsados: data.tokensUsados || null,
          tiempoGeneracionMs: data.tiempoGeneracionMs || null,
      ...
      if (error.code === 'P2002') {
        return reply.status(400).send({ error: 'DocumentoSeccion with this documento and orden already exists' });
      }
      if (error.code === 'P2003') {
        return reply.status(400).send({ error: 'Referenced documento does not exist' });
      }
      return reply.status(500).send({ error: 'Internal server error' });
    }
  });
  ...
  fastify.put('/:id', {
    ...
    try {
      ...
      const existingDocumentoSeccion = await prisma.documentoSeccion.findUnique({
        where: { id }
      });
      ...
      if (data.documentoId !== undefined) updateData.documentoId = data.documentoId;
      if (data.orden !== undefined) updateData.orden = data.orden;
      if (data.titulo !== undefined) updateData.titulo = data.titulo;
      if (data.descripcion !== undefined) updateData.descripcion = data.descripcion;
      if (data.contenido !== undefined) updateData.contenido = data.contenido;
      if (data.estado !== undefined) updateData.estado = data.estado;
      if (data.tokensUsados !== undefined) updateData.tokensUsados = data.tokensUsados;
      if (data.tiempoGeneracionMs !== undefined) updateData.tiempoGeneracionMs = data.tiempoGeneracionMs;
      if (data.articulosLcsp !== undefined) updateData.articulosLcsp = data.articulosLcsp;
  ...
  fastify.delete('/:id', {
    ...
    try {
      ...
      return reply.status(200).send({ message: 'DocumentoSeccion deleted successfully' });
    } catch (error) {
      return reply.status(500).send({ error: 'Internal server error' });
    }
  });
