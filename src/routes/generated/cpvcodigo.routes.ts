...
const routes: FastifyPluginAsync = async (fastify) => {
  ...
  fastify.get('/', {
    ...
    try {
      const { page = 1, limit = 10, codigo, descripcion, nivel, activo } = request.query as any;
  ...
  fastify.put('/:id', {
    ...
    try {
      ...
      if (data.codigoPadre && data.codigoPadre !== existingCpvCodigo.codigoPadre) {
        const parentExists = await prisma.cpvCodigo.findFirst({
          where: { codigo: data.codigoPadre }
        });
      ...
      if (data.codigoPadre !== undefined) updateData.codigoPadre = data.codigoPadre;
    ...
    try {
      ...
      if (childCodes) {
        return reply.status(400).send({ 
          error: 'Cannot delete CPV code with child codes. Delete child codes first.' 
        });
      }
...
1. **Complete CRUD operations** for the CpvCodigo model
2. **Pagination and filtering** on the list endpoint with search by código, descripción, nivel, and activo status
3. **Proper error handling** with specific HTTP status codes
4. **Authentication** using the authAccessToken plugin
5. **AJV schema validation** for request bodies and query parameters
6. **Swagger/OpenAPI documentation** with detailed schemas
7. **Business logic validation**:
   - Validates parent code exists when creating/updating
   - Prevents deletion of codes that have child codes
   - Handles unique constraint violations
8. **TypeScript types** and proper error responses
9. **Logging** for debugging purposes
10. **Optimized queries** with proper indexing considerations

The routes handle all edge cases and provide comprehensive error messages for better API usability.