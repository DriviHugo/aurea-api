...
const routes: FastifyPluginAsync = async (fastify) => {
  ...
  fastify.get('/', {
    ...
    try {
      const { page = 1, limit = 10, expedienteId, reglaId, passed } = request.query as any;
    ...
    schema: {
      ...
      body: {
        ...
        properties: {
          ...
          puntuacionEstructura: { type: ['integer', 'null'], minimum: 0, maximum: 100 },
          puntuacionContenido: { type: ['integer', 'null'], minimum: 0, maximum: 100 },
          usuarioId: { type: 'string', format: 'uuid' }
        }
      },
      response: {
  ...
  fastify.put('/:id', {
    ...
    try {
      ...
      for (const { type, result } of results) {
        if (!result) {
          return reply.status(400).send({ error: `${type.charAt(0).toUpperCase() + type.slice(1)} not found` });
        }
      }
      ...
      const validacion = await prisma.validacion.update({
        ...
        data: {
          ...
          ...(data.valorEncontrado !== undefined && { valorEncontrado: data.valorEncontrado }),
          ...(data.explicacion && { explicacion: data.explicacion }),
          ...(data.puntuacionEstructura !== undefined && { puntuacionEstructura: data.puntuacionEstructura }),
          ...(data.puntuacionContenido !== undefined && { puntuacionContenido: data.puntuacionContenido }),
