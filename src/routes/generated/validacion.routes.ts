...
const routes: FastifyPluginAsync = async (fastify) => {
  ...
  fastify.get('/', {
    ...
    try {
      const { page = 1, limit = 10, expedienteId, documentoId, reglaId, passed, usuarioId } = request.query as any;
  ...
  fastify.post('/', {
    ...
    try {
      ...
      const [expediente, documento, regla, usuario] = await Promise.all([
        prisma.expediente.findUnique({ where: { id: validacionData.expedienteId } }),
        validacionData.documentoId ? prisma.documento.findUnique({ where: { id: validacionData.documentoId } }) : null,
        prisma.regla.findUnique({ where: { id: validacionData.reglaId } }),
        prisma.profile.findUnique({ where: { id: validacionData.usuarioId } })
      ]);
