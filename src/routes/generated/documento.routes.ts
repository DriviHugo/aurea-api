...
const routes: FastifyPluginAsync = async (fastify) => {
  ...
  const documentoSchema = {
    ...
    properties: {
      ...
      tipo: {
        type: "string",
        enum: ["contrato", "acta", "informe", "resolucion", "dictamen", "otro"],
      },
    ...
    async (request, reply) => {
      try {
        ...
        if (error.code === "P2002") {
          return reply
            .status(400)
            .send({ error: "Documento with this data already exists" });
        }
        if (error.code === "P2003") {
          return reply
            .status(400)
      ...
      try {
        ...
        if (error.code === "P2003") {
          return reply.status(400).send({
            error: "Cannot delete documento due to existing references",
          });
        }
        return reply.status(500).send({ error: "Internal server error" });
      }
