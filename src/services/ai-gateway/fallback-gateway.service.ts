...
export class FallbackAIGatewayService {
  ...
  async complete(request: AICompletionRequest): Promise<AICompletionResponse> {
    ...
    if (forceAliaFail) {
      ...
      aliaError = new Error("ALIA forced failure for testing");
    }

    // 1. Intenta con ALIA (skip if forced fail)
    if (!forceAliaFail) {
      try {
    ...
    if (this.fallbackGateway) {
      logger.info({
        msg: "[FallbackAIGateway] ALIA failed, attempting fallback",
        aliaError: aliaError?.message,
      ...
      try {
        ...
        throw new Error(
          `Both AI providers failed. ALIA: ${aliaError?.message}. Fallback (${this.fallbackConfig.provider}): ${fbError.message}`,
        );
      }
    }
