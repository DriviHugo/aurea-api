...
async function runTests() {
  ...
  const unauthResult = await testEndpoint('/api/private/profile', 'GET');
  ...
  if (accessToken) {
    ...
    const authResult = await testEndpoint('/api/private/profile', 'GET', accessToken);
    results.push(authResult);
    console.log(
      authResult.success
        ? `   ✅ Profile access: ${authResult.status} - Got data!`
        : `   ❌ Profile access: ${authResult.status} - ${JSON.stringify(authResult.data)}`
    );

    // Test 6: Test a migrated endpoint (expediente)
    console.log('\n6️⃣  Testing migrated endpoint (expediente) WITH auth...');
    const expedienteResult = await testEndpoint(
      '/api/private/expediente',
      'GET',
      accessToken
    );
    results.push(expedienteResult);
    console.log(
      expedienteResult.success
        ? `   ✅ Expediente GET: ${expedienteResult.status} - Data: ${JSON.stringify(expedienteResult.data)}`
        : `   ⚠️  Expediente GET: ${expedienteResult.status} - ${JSON.stringify(expedienteResult.data)}`
    );

    // Test 7: Test another migrated endpoint (documento)
    console.log('\n7️⃣  Testing migrated endpoint (documento) WITH auth...');
    const documentoResult = await testEndpoint(
      '/api/private/documento',
      'GET',
      accessToken
    );
    results.push(documentoResult);
    console.log(
      documentoResult.success
        ? `   ✅ Documento GET: ${documentoResult.status} - Data: ${JSON.stringify(documentoResult.data)}`
        : `   ⚠️  Documento GET: ${documentoResult.status} - ${JSON.stringify(documentoResult.data)}`
    );
  } else {
  ...
  if (accessToken) {
    ...
    console.log('   - Migrated endpoints responding ✓');
  } else {
